...
$ifadeler = array(
  'Domain Name' => '<b>Alan Adı</b>',
  '** Registrant:' => '<b>** Kaydedici:</b><br />------------------------------------------------------',
  '** Administrative Contact:' => '<br /><b>** Yonetici Bilgileri</b><br />------------------------------------------------------',
  '** Technical Contact:' => '<br /><b>** Teknik Sorumlu Bilgileri</b><br />------------------------------------------------------',
  '** Billing Contact:' => '<br /><b>** Ödeme Sorumlu Bilgileri</b><br />------------------------------------------------------',
  '** Domain Servers:' => '<br /><b>** İsim Sunucuları:</b><br />------------------------------------------------------', 
  '** Additional Info:' => '<br /><b>** Diğer Bilgiler:</b><br />------------------------------------------------------',
  'Created on..............:' => 'Kayıt Tarihi............:',
  'Expires on..............:' => 'Bitiş Tarihi............:',
  'Registry Domain ID' => 'Kayıtlı Alan Adı Numarası',
  'Domain Name' => 'Alan Adı',
  'Registrar WHOIS Server' => 'Kaydeden WHOIS Serverı',
  'Registrar URL' => 'Kaydeden URL',
  'Updated Date' => 'Güncellenme Tarihi',
  'Creation Date' => 'Kaydedilme Tarihi',
  'Registrar Registration Expiration Date' => 'Bitiş Tarihi',
  'Registrar' => 'Kaydeden Firma',
  'Registrar IANA ID' => 'Kaydeden Firmanın IANA Numarası',
  'Registrar Abuse Contact Email' => 'Kaydeden Firma İletiþim Maili',
  'Registrar Abuse Contact Phone' => 'Kaydeden Firmanın İletiþim Telefonu',
  'Domain Status' => 'Alan Adı Durumu',
  'Registry Registrant ID' => 'Kaydedicinin Kayıt Numarası',
  'Registrant Name' => 'Kaydeden Firmanın Adı',
  'Registrant Street' => 'Kaydedici Firmanın Adresi',
  'Registrant City' => 'Kaydedici Firmanın Bulunduğu Şehir',
  'Registrant State/Province' => 'Kaydedici Firmanın Eyalet/İl',
  'Registrant Postal Code' => 'Kaydedici Firma Adresinin Posta Kodu',
  'Registrant Country' => 'Kaydedici Firmanın Bulunduğu Ülke',
  'Registrant Phone' => 'Kaydedici Firmanın Telefonu',
  'Registrant Fax' => 'Kaydedici Firmanın Fax Numarası',
  'Registrant Email' => 'Kaydedici Firmanın E-Posta Adresi',
  'Registry Admin ID' => 'Kaydeden Admin ID',
  'Admin Name' => 'Admin Adı',
  'Admin Organization' => 'Kaydeden URL',
  'Admin Street' => 'Adminin Adresi',
  'Admin State/Province' => 'Admin Eyalet/İl',
  'Admin City' => 'Admin Şehir',
  'Admin Postal Code' => 'Admin Posta Kodu',
  'Admin City' => 'Admin Şehir',
  'Admin Country' => 'Admin Ülke',
  'Admin Phone' => 'Admin Telefonu',
  'Admin Fax' => 'Admin Fax Numarası',
  'Admin Email' => 'Admin E-Mail Adresi',
  'Tech Name' => 'Teknik Sorumlunun Adı',
  'Tech Street' => 'Teknik Sorumlu Adresi',
  'Tech City' => 'Teknik Sorumlu Şehri',
  'Tech State/Province' => 'Teknik Sorumlu Eyalet/İl',
  'Tech Postal Code' => 'Teknik Sorumlu Posta Kodu',
  'Tech Country' => 'Teknik Sorumlu Ülkesi',
  'Tech Phone' => 'Teknik Sorumlunun Telefon Numarası',
  'Tech Fax' => 'Teknik Sorumlunun Fax Numarası',
  'Tech Email' => 'Teknik Sorumlunun E-Mail Adresi',
  'Name Server 1' => 'İsim Sunucusu 1',
...
function domain_sorgula($domain){
	...
	$sonuc = Whois_sorgula($whoissunucu, $domain);
	if(!$sonuc) {
		return "Error: No results retrieved from $whoissunucu server for $domain domain!";
	}
	else {
		while(strpos($sonuc, "Whois Server:") !== FALSE){
			preg_match("/Whois Server: (.*)/", $sonuc, $matches);
			$secondary = $matches[1];
			if($secondary) {
				$sonuc = Whois_sorgula($secondary, $domain);
				$whoissunucu = $secondary;
			}
		}
	}
	return "$domain domain lookup results from $whoissunucu server:\n\n" . $sonuc;
}

function ip_sorgula($ip) {
	$whoissunuculari = array(
	...
	$sonucs = array();
	foreach($whoissunuculari as $whoissunucu) {
		$sonuc = Whois_sorgula($whoissunucu, $ip);
		if($sonuc && !in_array($sonuc, $sonucs)) {
			$sonucs[$whoissunucu]= $sonuc;
		}
	}
	$res = "RESULTS FOUND: " . count($sonucs);
	foreach($sonucs as $whoissunucu=>$sonuc) {
		$res .= "\n\n-------------\nLookup results for " . $ip . " from " . $whoissunucu . " server:\n\n" . $sonuc;
	}
	return $res;
}

function ip_dorgula($ip) {
	$ipnums = explode(".", $ip);
	if(count($ipnums) != 4) {
		return false;
	}
...
function domain_dorgula($domain) {
...
function Whois_sorgula($whoissunucu, $domain) {
	$port       = 43;
	$timeout    = 10;
	$fp         = @fsockopen($whoissunucu, $port, $errno, $errstr, $timeout) or die("Socket Error " . $errno . " - " . $errstr);
